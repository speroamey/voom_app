...
/** Variable: strip
  ...
  StanzaBuilder _buildBody() {
    StanzaBuilder bodyWrap = Strophe
        .$build('body', {'rid': this.rid++, 'xmlns': Strophe.NS['HTTPBIND']});
    if (this.sid != null) {
  ...
  _restore(String jid, Function callback, int wait, int hold, int wind) {
    var session = JSON.decode(SessionStorage.getItem('strophe-bosh-session'));
    if (session != null &&
        session.rid &&
        session.sid &&
        session.jid &&
  ...
  _onRequestStateChange(Function func, StropheRequest req) {
    ...
    if (valid_request || too_many_retries) {
      ...
      Strophe
          .debug("request id " + req.id.toString() + " should now be removed");
    }
